...
delete_file(service, '1g0N-pbai3hNuVKol_vxGypGpdLEniVWCdEUky_8RZGQ')
# delete_file(service, '1v92imc5F4_bsygRHTKKb-HKCxBL79XZB')
...
file_id = "1xrdbX2HUiy2KZMw7kXTeHiBBkFAbJku8"
# f = open("myfile.txt", "r+b")
...
def test():
    ...
    res = service.files().create(
        ...
        media_body=MediaFileUpload("myfile.jpg", supportsAllDrives="true", mimetype='application/vnd.google-apps.document', resumable=True)
    ).execute()


    # downloader = MediaIoBaseDownload(
    #     io.FileIO("myfile.txt", 'wb'),
    #     service.files().export_media(fileId=file_id, mimeType='application/vnd.google-apps.document')
    # )
    # done = False
    # while done is False:
