...
def transfer_files():
    jpg_transfer = service.files().update(fileId=jpg_file_id,
                                    addParents=COMPLETED_FOLDER_ID,
                                    removeParents=PENDING_REVIEW_FOLDER_ID,
                                    fields='id, parents').execute()

    txt_transfer = service.files().update(fileId=txt_file_id,
                                    addParents=COMPLETED_FOLDER_ID,
                                    removeParents=PENDING_REVIEW_FOLDER_ID,
                                    fields='id, parents').execute()
...
def decrement_review_counter(file_id):
    if int(get_file_description(file_id)) >= -2:
